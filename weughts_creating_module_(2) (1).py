# -*- coding: utf-8 -*-
"""Weughts_creating_module (2).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1xMG3Y9oZM6dt9JFIkZNMatpB_riJAOQB
"""
...
df = pd.read_csv("/content/gdp.csv", sep = ";")
...
# Создаем список стран от 0 до 99
countries = list(range(100))

# Генерируем 100 случайных целых чисел для столбца 'gdp' в диапазоне от 4e8 до 3e13
gdp = [random.randint(4e8, 3e13) for _ in range(100)]

# Создаем датафрейм
df = pd.DataFrame({'countries': countries, 'gdp': gdp})

# Выводим первые несколько строк датафрейма
print(df.head(50))

df.to_csv('gdp.csv', sep=';', index=False)

import pandas as pd
import random
country = list(range(100))
year = ["2021"] * 100
ppc = [random.uniform(0, 100) for _ in range(100)]
unnamed = ["" for _ in range(100)]
norm_ppc = [random.uniform(0, 1) for _ in range(100)]
df2 = pd.DataFrame({'country': country, 'year': year, 'ppc': ppc, 'Unnamed': unnamed, 'norm_ppc': norm_ppc})
print(df2.head())
...
# Генерируем все возможные комбинации пар стран
combinations = [(i, j) for i in countries for j in countries]

# Создаем столбец 'year' со значениями "2021"
year = ["2021"] * len(combinations)

# Генерируем случайные значения для столбцов 'v', 'vright', 'sharev' и 'sharev_norm'
data = {
    'year': year,
    'i': [i for i, _ in combinations],
    'j': [j for _, j in combinations],
    'v': [random.uniform(0, 2e6) if i != j and random.random() > 0.1 else 0 for i, j in combinations],
    'vright': [random.uniform(0, 2e6) if i != j and random.random() > 0.1 else 0 for i, j in combinations],
    'sharev': [random.uniform(0, 13) if i != j and random.random() > 0.1 else 0 for i, j in combinations],
    'sharev_norm': [random.uniform(0, 1) if i != j and random.random() > 0.1 else 0 for i, j in combinations]
}
...
df3.to_csv("/content/trade_sharev_norm.csv",sep = ";",index=False)
